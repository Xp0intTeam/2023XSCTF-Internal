...
int main() {
    ...
    int i;
    char temp[4];
    int part, len, idx,enc_len;
    char *enc;
    memset(str, 0, sizeof(str));
    scanf("%s", str); //flag{1_l0ve_reve25e} len==20
    len = strlen(str);
    part = len / 3;
    if (len == 20) {
        for (i = 0, idx = 0; i < part + 1; ++i, idx += 3) {
            memset(temp, 0, sizeof(temp));
            memcpy(temp, str + idx, 3);
            enc = (char*)encrypt((unsigned char *)temp);
            enc_len=strlen(enc);
            get_trans((unsigned char*)enc);
